...
// helper functions
  ...
  if(Session.get("Role") == "userDash" && Session.get("StockChartItem") != undefined){
    ...
    Tracker.autorun(function () {
      var gp = Events.find({$and: [{"itemNo": Session.get("StockChartItem")}, {"gameCode": Session.get("GameCode")}, {"key": "StockPriceChange"}, {"group": Session.get("GroupNo")}]}, {sort: {"timestamp": 1}}).map(function (u) {return u.newPrice});
      goldPrices = gp;
      currentLength = goldPrices.length;
